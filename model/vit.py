...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x_attn = self.multi_head_attention(self.norm1(x))
...
class ViT(nn.Module):
    ...
    def forward(self, X: torch.Tensor) -> torch.Tensor:
        ...
        all_tokens = torch.cat([cls_tokens, embedded_patches], dim=1)
        
        # add the positional embedding to the token sequence
        pos_embed = self.pos_embed.repeat(B, 1, 1)
        all_tokens = all_tokens + pos_embed

        # run the positionaly embedded tokens through all transformer blocks 
